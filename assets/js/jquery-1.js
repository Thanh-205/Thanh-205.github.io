...
(function (A, w) {
            ...
            if (p.length > 1 && r.exec(g))
                ...
                if (h) {
                    v = m
                        ...
                        : k.find(
                              p.pop(),
                              p.length === 1 && (p[0] === '~' || p[0] === '+') && h.parentNode ? h.parentNode : h,
                              M,
                          );
                ...
                n.find.ID = function (m, q, p) {
                    if (typeof q.getElementById !== 'undefined' && !p)
                        return (q = q.getElementById(m[1]))
                            ? q.id === m[1] ||
                              (typeof q.getAttributeNode !== 'undefined' && q.getAttributeNode('id').nodeValue === m[1])
                                ? [q]
                                : w
                            : [];
                };
        ...
        domManip: function (a, b, d) {
            function f(u) {
                return c.nodeName(u, 'table')
                    ? u.getElementsByTagName('tbody')[0] || u.appendChild(u.ownerDocument.createElement('tbody'))
                    : u;
            }
            var e,
                j,
    ...
    c.extend({
            ...
            if (d)
                for (j = 0; e[j]; j++)
                    if (
                        ...
                        e[j].nodeType === 1 &&
                            e.splice.apply(e, [j + 1, 0].concat(c.makeArray(e[j].getElementsByTagName('script'))));
                        d.appendChild(e[j]);
                    }
            return e;
        },
        cleanData: function (a) {
            for (
                var b, d, f = c.cache, e = c.event.special, j = c.support.deleteExpando, i = 0, o;
                (o = a[i]) != null;
                i++
            )
                if ((d = o[c.expando])) {
    ...
    c.fn.extend({
        ...
        serializeArray: function () {
            return this.map(function () {
                ...
                .filter(function () {
                    return (
                        this.name && !this.disabled && (this.checked || ub.test(this.nodeName) || vb.test(this.type))
                    );
                })
                .map(function (a, b) {
                    a = c(this).val();
    ...
    c.extend({
        ...
        ajaxSettings: {
            ...
            async: true,
            xhr:
                A.XMLHttpRequest && (A.location.protocol !== 'file:' || !A.ActiveXObject)
                    ? function () {
            ...
            function d(i, o) {
                if (c.isArray(o))
                    c.each(o, function (k, n) {
                        b || /\[\]$/.test(i)
                            ? f(i, n)
                            : d(i + '[' + (typeof n === 'object' || c.isArray(n) ? k : '') + ']', n);
                    });
                else
    ...
    c.fx.prototype = {
        ...
        step: function (a) {
            ...
            if (a || b >= this.options.duration + this.startTime) {
                ...
                this.pos = c.easing[(this.options.specialEasing && this.options.specialEasing[this.prop]) || a](
                    this.state,
                    e,
                    0,
                    1,
            ...
            if (f !== w)
                ...
                return (j = wa(e))
                    ? 'pageXOffset' in j
                        ...
                        : (c.support.boxModel && j.document.documentElement[d]) || j.document.body[d]
                    : e[d];
        };
    });
